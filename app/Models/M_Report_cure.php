...
class M_Report_cure extends Model
{
    //table = deklarasikan nama tabel yg akan dipanggil
    protected $bf_cure = "bf_cure";
    ...
    public function __construct()
    {
        ...
        //deklartabel
        $this->db_rpot = $this->db->table($this->bf_cure);
    ...
    function _get_datatables()
    {
        ...
        if($f1)
		{
            ...
            $this->db_rpot->where("CAST(CURE_TIME AS DATE)>=", $tgl1);
            $this->db_rpot->where("CAST(CURE_TIME AS DATE)<=", $tgl2);
		}
        ...
        $f2=$this->request->getPost("f2");
...

		 {
             if($f2==1)
             {
                $this->db_rpot->where("CAST(CURE_TIME AS TIME) BETWEEN '00:00' and '08:00'", NULL, FALSE ); 
             }
			
             if($f2==2)
             {
                $this->db_rpot->where("CAST(CURE_TIME AS TIME) BETWEEN '08:00' and '16:00'", NULL, FALSE ); 
             }
			
             if($f2==3)
             {
                $this->db_rpot->where("CAST(CURE_TIME AS TIME) BETWEEN '16:00' and '23:59'", NULL, FALSE ); 
             }

		}
		

        $column_order = array('MM_CODE', 'MAT_IP_CODE', 'MAT_DESC', 'Amount', 'Park', 'CURE_TIME', 'cured_stts', 'actual_cure'); //field yang ada di table
        $column_search = array('MAT_IP_CODE','Amount');
